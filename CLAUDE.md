...
2. **sub_input** — אופציה יכולה להכריז על `sub_input:{id,label,placeholder}`. כשהיא נבחרת, מופיע inline שדה טקסט; הערך נשמר ב-`variables[questionId].sub_inputs[sub_input.id]`. נתמך ב-`select_with_custom`, `single_select`, ו-`multi_select` — בזיהוי אוטומטי. אם אחת מהאופציות של single_select מכריזה על sub_input, ה-value אוטומטית הופך מ-string לאובייקט `{selected, sub_inputs}`. עבור multi_select, ה-value הופך מ-array לאובייקט `{selected:[], sub_inputs:{}}`. ביטול סימון של אופציה עם sub_input מנקה את ה-sub_input שלה.
...
- `src/config/sessionTypes/` — קונפיגים פר-סשן: `anticipatoryAnxiety.js`, `parentalAnger.js`, `relationshipConflict.js`, `rumination.js`, `rejection.js`, `boundaryFailure.js`, `loneliness.js` — שבעת הסשנים מהרשימה המקורית הושלמו. `_placeholder` הוסר ב-2026-05-19 לאחר שכל 7 הסשנים פעילים. helpers משותפים ב-`_promptHelpers.js`. כל סשן עומד בפני עצמו.
